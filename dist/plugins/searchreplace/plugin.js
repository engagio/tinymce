...
var searchreplace = (function () {
  ...
  var $_9tjcmhijjdmdg1vc = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_9tjcmhijjdmdg1vc.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_g0r152ihjdmdg1v6.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_g0r152ihjdmdg1v6.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_g0r152ihjdmdg1v6.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_g0r152ihjdmdg1v6.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_g0r152ihjdmdg1v6.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_g0r152ihjdmdg1v6.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_g0r152ihjdmdg1v6.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_g0r152ihjdmdg1v6.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_g0r152ihjdmdg1v6.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_g0r152ihjdmdg1v6.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_g0r152ihjdmdg1v6.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_g0r152ihjdmdg1v6.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_g0r152ihjdmdg1v6.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_g0r152ihjdmdg1v6.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_g0r152ihjdmdg1v6.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_f833tgiljdmdg1vj = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_f833tgiljdmdg1vj.open(editor, currentIndexState);
    });
  };
  var $_1h556tikjdmdg1vh = { register: register };
  ...
  var $_4mvsu9imjdmdg1vn = { register: register$1 };

  PluginManager.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_1h556tikjdmdg1vh.register(editor, currentIndexState);
    $_4mvsu9imjdmdg1vn.register(editor, currentIndexState);
    return $_8sd8uyigjdmdg1v3.get(editor, currentIndexState);
  });
  function Plugin () {
  }
