...
var textpattern = (function () {
  ...
  var $_4utvjsq5jdmdg41l = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_ctyfytqdjdmdg420.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_ctyfytqdjdmdg420.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_ctyfytqdjdmdg420.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_7ak0bcqajdmdg41u.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_7ak0bcqajdmdg41u.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_7ak0bcqajdmdg41u.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !VK.modifierPressed(e)) {
        $_2kwo1aq9jdmdg41q.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_2kwo1aq9jdmdg41q.checkKeyCode(keyCodes, e)) {
        $_2kwo1aq9jdmdg41q.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_2kwo1aq9jdmdg41q.checkCharCode(charCodes, e)) {
  ...
  var $_6npfcjq6jdmdg41n = { setup: setup };

  PluginManager.add('textpattern', function (editor) {
    var patternsState = Cell($_4utvjsq5jdmdg41l.getPatterns(editor.settings));
    $_6npfcjq6jdmdg41n.setup(editor, patternsState);
    return $_dtlzy9q4jdmdg41k.get(patternsState);
  });
  function Plugin () {
  }
